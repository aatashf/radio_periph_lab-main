...
int main()
{
    ...
    init_platform();
    print("\r\n\r\n\r\nLab 6 Atousa Atashfaraz - Custom Peripheral Demonstration\n\r");
    print("Configuring Codec Now\r\n");
    configure_codec();
    radioTuner_controlReset(XPAR_RADIO_TUNER_0_S00_AXI_BASEADDR, 0x1);
    ...
    start_time = RADIO_TUNER_mReadReg(XPAR_RADIO_TUNER_0_S00_AXI_BASEADDR, RADIO_TUNER_S00_AXI_SLV_REG3_OFFSET);
    for (i=0;i<1024;i++)
        stop_time = RADIO_TUNER_mReadReg(XPAR_RADIO_TUNER_0_S00_AXI_BASEADDR, RADIO_TUNER_S00_AXI_SLV_REG3_OFFSET);
    printf("Elapsed time in clocks = %u\n",stop_time-start_time);
    float throughput=0;
    // please insert your code here for calculate the actual throughput in Mbytes/second
    float tics=stop_time-start_time;
    float elapsedtime=tics/125e6;
    throughput = (1024*4)/elapsedtime; //4bytes * 1024 reads
    // how much data was transferred? How long did it take?
    printf("Estimated Transfer throughput = %f Mbytes/sec",throughput/1e6);
#endif
